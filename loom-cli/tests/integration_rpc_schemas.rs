...
//! Covers the CLI side: `loom action rpc.schemas` must reach the
//! daemon, not be rejected at the CLI's own `validate_args` gate.
//!
...
//! dogfood bug surfaced ('unknown method: rpc.schemas').
...
// ── `loom action rpc.schemas` reaches the wire ──────────────────────────────

/// Dogfood regression: `loom action rpc.schemas --session $S` returned
...
/// test pins the fix.
#[test]
fn test_rpc_schemas_passes_cli_validate_args() {
    ...
    validate_args(&schemas, "rpc.schemas", &args).expect(
        "rpc.schemas must pass CLI validate_args so the \

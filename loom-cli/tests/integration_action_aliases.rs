...
//!
//! rpc.schemas advertising aliases is covered by
//! `loom-rpc/tests/integration_action_routing.rs`.
//!
//! Test pattern mirrors `integration_action_errors.rs`:
...
//!   surfaced as the dogfood "unknown method: web.type-text" bug.
...
#[test]
fn test_web_type_canonical_accepted() {
    let (schemas, _dir) = schema_cache_with_web_type();
    let args = well_formed_args();
    validate_args(&schemas, "web.type", &args)
        .expect("web.type must be accepted (no 'unknown method')");
}

// ── `web.type_text` continues to work via alias ─────────────────────────────

#[test]
fn test_web_type_text_alias_accepted() {
    let (schemas, _dir) = schema_cache_with_web_type();
    let args = well_formed_args();
    validate_args(&schemas, "web.type_text", &args)
        .expect("web.type_text must be accepted as an alias for web.type");
}

// ── `web.type-text` rejected with hint ──────────────────────────────────────

#[test]
fn test_web_type_kebab_rejected_with_hint_for_canonical() {
    let (schemas, _dir) = schema_cache_with_web_type();
    let args = well_formed_args();
    let err = validate_args(&schemas, "web.type-text", &args)
        .expect_err("web.type-text must be rejected");
...
#[test]
fn test_web_type_kebab_rejected_with_hint_for_alias() {
    let (schemas, _dir) = schema_cache_with_web_type();
    let args = well_formed_args();
    let err = validate_args(&schemas, "web.type-text", &args)
        .expect_err("web.type-text must be rejected");
...
#[test]
fn test_kebab_rejection_exit_code_is_2() {
    let (schemas, _dir) = schema_cache_with_web_type();
    let args = well_formed_args();
...
// ── negative control: still errors on a totally-unrelated method

#[test]
fn test_unknown_unrelated_method_still_errors() {
    let (schemas, _dir) = schema_cache_with_web_type();
    let args = well_formed_args();
...
// ── every alias target exists in BUILTIN_SCHEMAS ────────────────────────────
...
#[test]
fn test_every_alias_target_appears_in_builtin_schemas() {

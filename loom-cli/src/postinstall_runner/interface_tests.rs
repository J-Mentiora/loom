...
// macOS-only: `LaunchdPlistWriter::write` is a stub on every other target
// (returns `Internal("launchd plist is macOS-only")` before reaching the
// PermissionDenied path this test exercises). Unix-wide gating was wrong.
#[cfg(target_os = "macos")]
#[test]

"""
Hand-written admin payload types — NOT generated by ``loom._codegen``.
...
to the schema-driven codegen). These dataclasses mirror the Rust structs
in ``loom-rpc/src/rpc_handlers/rpc_handlers.rs`` and are kept in sync via
the drift test in ``tests/test_codegen_drift.py::test_handwritten_admin_types_field_drift``.

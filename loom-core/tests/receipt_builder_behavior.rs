...
fn ac_core_04_1_click_receipt_default_tier() {
    ...
    assert!(receipt.dom_after_hash.is_some(), "dom_after_hash must be present for click tier");
    assert!(receipt.screenshot_after_hash.is_some(), "screenshot_after_hash must be present");

    // NO blob fields for default click tier
    assert!(receipt.dom_after_blob_ref.is_none(), "click tier must NOT have dom_after_blob_ref");
    assert!(receipt.screenshot_after_blob_ref.is_none(), "click tier must NOT have screenshot_after_blob_ref");
    assert!(receipt.dom_before_blob_ref.is_none(), "no before-refs in default tier");
    assert!(receipt.screenshot_before_blob_ref.is_none());

    // No navigate-only fields
    assert!(receipt.network_events.is_empty(), "click tier has no network_events");
    assert!(receipt.console_lines.is_empty(), "click tier has no console_lines");
    assert!(receipt.return_value_json.is_none(), "no return_value_json for click");
    assert!(receipt.return_value_blob_ref.is_none(), "no return_value_blob_ref for click");
}
...
fn ac_core_04_2_navigate_receipt_default_tier() {
    let dom_ref = ContentRef { sha256: "a".repeat(64), size_bytes: 4096 };
    let ss_ref = ContentRef { sha256: "b".repeat(64), size_bytes: 1024 };
    let net_event = NetworkEvent {
        ...
        response_body_ref: Some(ContentRef { sha256: "c".repeat(64), size_bytes: 512 }),
        timing_ticks: 1_000,
    ...
    // Full blob refs
    assert!(receipt.dom_after_blob_ref.is_some(), "navigate tier needs dom_after_blob_ref");
    assert!(receipt.screenshot_after_blob_ref.is_some(), "navigate tier needs screenshot_after_blob_ref");
    assert_eq!(receipt.dom_after_blob_ref.as_ref().unwrap().sha256, "a".repeat(64));
    ...
    assert!(receipt.dom_after_hash.is_none(), "navigate tier uses blob-refs not hash fields");
...
fn ac_core_04_3_evaluate_receipt_default_tier() {
    ...
    assert!(receipt.dom_after_blob_ref.is_none(), "evaluate has no DOM blob");
    assert!(receipt.screenshot_after_hash.is_none());
    assert!(receipt.screenshot_after_blob_ref.is_none());
    assert!(receipt.network_events.is_empty(), "evaluate has no network_events");
}
...
fn ac_core_04_4_capture_full_adds_before_refs() {
    let dom_before = ContentRef { sha256: "e".repeat(64), size_bytes: 2048 };
    let ss_before = ContentRef { sha256: "f".repeat(64), size_bytes: 512 };
    ...
    assert!(receipt.dom_before_blob_ref.is_some(), "Full capture must include dom_before_blob_ref");
    assert!(receipt.screenshot_before_blob_ref.is_some(), "Full capture must include screenshot_before_blob_ref");

    // Hash-only after-fields must STILL be present (Full doesn't replace them)
    assert!(receipt.dom_after_hash.is_some(), "Full capture must NOT remove dom_after_hash for click tier");
    assert!(receipt.screenshot_after_hash.is_some(), "Full capture must NOT remove screenshot_after_hash");

    // No blob-ref for after in click (hash-only)
    assert!(receipt.dom_after_blob_ref.is_none(), "click tier keeps hash-only after fields even under Full");
}
...
fn ac_core_04_5_capture_minimal_strips_blobs() {
    ...
    assert!(receipt.dom_after_blob_ref.is_none(), "Minimal strips dom_after_blob_ref");
    assert!(receipt.screenshot_after_blob_ref.is_none(), "Minimal strips screenshot_after_blob_ref");
    assert!(receipt.dom_before_blob_ref.is_none(), "Minimal strips dom_before_blob_ref");
    assert!(receipt.screenshot_before_blob_ref.is_none(), "Minimal strips screenshot_before_blob_ref");

    // Hash fields must be present (extracted from ContentRef.sha256)
    assert_eq!(receipt.dom_after_hash.as_deref(), Some("a".repeat(64).as_str()));
    assert_eq!(receipt.screenshot_after_hash.as_deref(), Some("b".repeat(64).as_str()));

    // Network events: body refs stripped, hash/metadata preserved
    assert_eq!(receipt.network_events.len(), 1, "Network events array stays (just body refs stripped)");
    assert!(receipt.network_events[0].response_body_ref.is_none(), "Minimal strips body ref from events");
    assert!(!receipt.network_events[0].response_body_sha256_hex.is_empty(), "Hash preserved in minimal");

    // console_lines empty
    assert!(receipt.console_lines.is_empty(), "Minimal empties console_lines");
}
...
fn ac_core_05_1_error_receipt_shape() {
    ...
    assert!(receipt.details.is_none(), "optional details absent when not passed");
...
fn ac_core_05_1_error_receipt_message_truncation() {
    ...
    let msg = receipt.message.as_ref().unwrap();
    assert_eq!(msg.len(), 280, "message must be truncated to exactly 280 chars");
}
...
fn ac_core_05_2_all_receipt_codes_have_wire_strings() {
    ...
    for code in &codes {
        ...
        assert!(wire.chars().all(|c| c.is_ascii_lowercase() || c == '_'),
            "wire string must be snake_case: {}", wire);
    }
}
...
fn ac_core_05_2_receipt_code_serde_matches_as_wire() {
    ...
    for code in codes {
        ...
        assert_eq!(serialized, expected,
            "serde output {:?} must match as_wire() {:?}", serialized, expected);
    }
}
    ...
    match v {
        serde_json::Value::Number(n) => {
            assert!(n.is_i64() || n.is_u64(),
                "receipt must not contain float values, found: {}", n);
        }
...
fn ac_nfr_det_04_1_canonical_field_ordering() {
    ...
    let bytes1 = receipt.canonical_bytes().expect("canonical_bytes must succeed");
    ...
    sorted_keys.sort();
    assert_eq!(keys, sorted_keys, "JSON keys must be in lexicographic order");
}
...
fn ac_nfr_det_05_1_timing_ticks_is_integer() {
    ...
    if let Some(v) = json.get("timing_ticks") {
        assert!(v.is_u64() || v.is_i64(), "timing_ticks must serialize as integer, got: {}", v);
    }
}
...
fn ac_nfr_det_05_1_timing_ticks_monotonic_in_sequence() {
    ...
    let t1 = 1_000_u64;
    let t2 = 2_000_u64;
    assert!(t1 >= t0 && t2 >= t1, "timing_ticks must be monotonically non-decreasing");
    // The builder accepts u64, which is always an integer.
    let r0 = ReceiptBuilder::build_click_receipt("a0".to_string(), t0, "0".repeat(64), "1".repeat(64));
    let r1 = ReceiptBuilder::build_click_receipt("a1".to_string(), t1, "0".repeat(64), "1".repeat(64));
    let r2 = ReceiptBuilder::build_click_receipt("a2".to_string(), t2, "0".repeat(64), "1".repeat(64));

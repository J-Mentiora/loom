...
use loom_core::determinism_harness::{
    DeterminismHarness, SideEffectTape, TapeFrame,
};
...
fn test_clock_now_frozen_within_action() {
    ...
    for _ in 1..100 {
        assert_eq!(h.clock_now(), first, "clock_now() must return same value on every call within an action");
    }
}
...
fn test_clock_now_returns_milliseconds_not_nanoseconds() {
    ...
    assert!(ms < 10_000_000_000_000, "clock value should be in milliseconds: got {ms}");
}
...
fn test_begin_action_advances_clock() {
    ...
    assert_eq!(after, before + 10, "begin_action(10) should advance clock by 10 ms");
}
...
fn test_rng_same_seed_identical_sequence() {
    ...
    assert_eq!(seq1, seq2, "Same seed must produce identical 100-element sequence");
}
...
fn test_canonicalize_rfc8785_key_ordering() {
    ...
    assert!(a_pos < m_pos && m_pos < z_pos, "JCS key ordering violated: got {s}");
}
...
fn test_hash_canonical_is_64_char_hex() {
    ...
    assert_eq!(hex.len(), 64, "SHA-256 hex must be 64 chars, got len {}", hex.len());
    assert!(hex.chars().all(|c| c.is_ascii_hexdigit()), "must be lowercase hex: {hex}");
}
...
fn test_replay_table_pop_clock_returns_recorded_value() {
    ...
    let val = table.pop_clock().expect("pop_clock should succeed on ClockRead frame");
    assert_eq!(val, 12345);
}
...
fn test_replay_table_pop_rng_returns_recorded_value() {
    let h = make_harness(0);
    let tape = SideEffectTape {
        frames: vec![TapeFrame::RngDraw { value_u64: 0xdeadbeef }],
    };
    let table = h.install_replay_mode(tape);
    let val = table.pop_rng().expect("pop_rng should succeed on RngDraw frame");
...
fn test_replay_table_pop_net_returns_frame() {
    ...
    if let TapeFrame::NetResponse { request_id, status, .. } = frame {
...
fn test_replay_table_pop_clock_wrong_frame_returns_err() {
    ...
    assert!(result.is_err(), "pop_clock on RngDraw frame must return an error");
...
fn test_replay_table_tape_exhausted_returns_err() {
    ...
    let table = h.install_replay_mode(tape);
    let result = table.pop_clock();
    assert!(result.is_err(), "pop_clock on empty tape must return an error");
}
...
fn test_tape_writer_snapshot_is_stable_clone() {
    ...
    assert_eq!(snap2.frames.len(), 2, "second snapshot should have 2 frames");
}

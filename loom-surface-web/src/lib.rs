...
fn navigate_verb(a: Action) -> Result<Receipt, HostError> {
    ...
    Ok(Receipt {
        ...
        // Observational network-entries side-channel — pass through the
        // host's inline-XOR-blob decision + truncation flag verbatim.
        // NOT folded into outcome_hash (excluded from the replay chain).
        network_entries_json: result.network_entries_json,
        network_entries_blob_ref: result.network_entries_blob_ref,
        network_entries_truncated: Some(result.network_entries_truncated),

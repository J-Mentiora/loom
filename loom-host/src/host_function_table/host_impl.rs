...
// This is the sole WASM↔core bridge (IC-HOST-03). Mode dispatch (live vs
...
impl Host for HostState {
    ...
    // --- Network (vault-mediated; IC-HOST-04 HARD) ---
    ...
    fn net_request(
        ...
        async move {
            ...
            // IC-HOST-04 HARD: strip Authorization from response headers
    ...
    fn shim_call(
        ...
        // gets its own subprocess (AC-SHCRT-04 reuse falls out: same
        ...
        // AC-SAFEPROF-04: capture profile + downloads_dir for env-var
                ...
                Ok((effective_id, msg, shim_manager)) => {
                    ...
                    if !shim_manager.is_registered(&effective_id) {
                            ...
                            .map(|c| c.clone())
                        {
                            ...
                            // AC-SAFEPROF-04: inject profile + downloads_dir
                            // env vars so the shim's CDP bootstrap can send
                                ...
                                match &downloads_dir_for_register {
                                    ...
                                    None => {
                                        ...
                                        tracing::error!(
                                            ...
                                            "AC-SAFEPROF-04 invariant: safe profile session has no \
    ...
    fn log_emit(
        ...
        // Route through HostObservability in Phase 3.2 full impl.
        async move {}
    }
    ...
    // --- Typed navigate-execute host function (AC-NAVRECEIPT-01..05) ---
    ...
    fn navigate_execute(
        ...
        // AC-BLOCKLIST-04: compute affirmative wire form. The session
        ...
        // AC-SAFEPROF-04: same env-var contract as `shim_call`. Captured
                            ...
                            match &downloads_dir_for_register {
                                ...
                                None => {
                                    ...
                                    tracing::error!(
                                        ...
                                        "AC-SAFEPROF-04 invariant: safe profile session has no downloads_dir; \
        ...
        async move {
            ...
            // AC-BLOCKLIST-02: every blocked sub-resource becomes a
            ...
            // AC-NAVERR-01..03 / AC-NETERR-01..04: surface typed network
            // failures as a typed HostError. Order matters:
            ...
            // AC-HAREXPORT-03 (P0): the shim has captured the
                ...
                .find(|e| e.error_reason.is_some())
            {
                ...
                let detail = serde_json::json!({
                    "kind": kind,
                    "url": url, // AC-NAVERR-03 / AC-NETERR-02 — navigate target
            ...
            // Store dom_bytes and screenshot_bytes in ContentStore (AC-NAVRECEIPT-04).
            ...
            // Session-monotonic timestamp from DeterminismHarness (AC-NAVRECEIPT-02).
            let emitted_at_ms = determinism.clock_now();

            // Serialize network events as JSON Vec<LoomNetworkEvent> (AC-NAVRECEIPT-03).
            let side_effects_json = serde_json::to_vec(&outcome.network_events).unwrap_or_default();

            // Brief AC-NAVRECEIPT2-01 extension: serialize console_lines verbatim
            // (Phase 6 stub returns Vec::new() until shim console capture lands).
            let console_lines_json = serde_json::to_vec(&outcome.console_lines).unwrap_or_default();

            // Brief AC-NAVRECEIPT2-01 extension: aggregate NetworkSummary from
            // captured events. Errors counted as HTTP 4xx/5xx OR transport
            // failures (`error_reason.is_some()`).
    ...
    // --- Typed evaluate-execute host function (AC-EVALRESULT-01..04) ---
    ...
    fn evaluate_execute(
        ...
        async move {
            ...
            // target_id = 0 (Phase 6 stub) — send_evaluate will lazy-spawn
            ...
            // AC-EVALRESULT-03: page-side throw → typed shaped error.
            ...
            // Session-monotonic timestamp (AC-NAVRECEIPT-02 parallel).
            let emitted_at_ms = determinism.clock_now();

            // AC-EVALRESULT-04: > 64KB → offload to content store.

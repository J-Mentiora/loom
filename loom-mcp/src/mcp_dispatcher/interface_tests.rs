...
#[tokio::test]
async fn implicit_session_create_forwards_audio() {
    // voice-call-io: a truthy `LOOM_MCP_SESSION_AUDIO` baseline must reach
    // session.create as `audio:true` (browser voice call). `None`/`Some(false)`
    // must NOT emit the key (byte-identical default shape).
    let options = SessionOptions {
        audio: Some(true),
        ..SessionOptions::default()
    };
    ...
    assert_eq!(
        creates,
        vec![json!({ "profile": "standard", "audio": true })],
        "audio baseline must reach session.create"
    );
}
...
fn session_options_parse_valid_invalid_and_empty_values() {
    assert_eq!(
        SessionOptions::from_values(None, None, None, None, None, None).unwrap(),
        SessionOptions::default()
    );
    assert_eq!(
    ...
    let err =
        SessionOptions::from_values(Some("not-a-number".into()), None, None, None, None, None)
            .unwrap_err();
    assert_eq!(err.code, LoomErrorCode::InvalidArgument);
    assert!(err.message.contains("LOOM_MCP_SESSION_SEED"), "{err}");
    let err =
        SessionOptions::from_values(None, Some("-5".into()), None, None, None, None).unwrap_err();
    assert!(
    ...
    let err = SessionOptions::from_values(None, None, None, Some("{not json".into()), None, None)
        .unwrap_err();
    assert_eq!(err.code, LoomErrorCode::InvalidArgument);
    ...
    let err = SessionOptions::from_values(None, None, None, None, Some("maybe".into()), None)
        .unwrap_err();
    assert_eq!(err.code, LoomErrorCode::InvalidArgument);
    assert!(
    ...
    // voice-call-io: audio parses from its env var and forwards only when on.
    assert_eq!(
        SessionOptions::from_values(None, None, None, None, None, Some("1".into()))
            .unwrap()
            .audio,
        Some(true)
    );
    assert_eq!(
        ...
        .to_create_params(),
        json!({ "profile": "standard", "audio": true })
    );
}

...
**Platforms.** macOS arm64/x86_64 and Linux x86_64/arm64. Windows is not supported on v0.13.2.
...
| CLI + runtime | Cargo (from git) | `loom-cli` | `cargo install --git … --tag v0.13.2 loom-cli` |
...
```bash
cargo install --git https://github.com/mentiora-ai/loom --tag v0.13.2 loom-cli
loom postinstall
loom doctor
```
...
```bash
pip install "git+https://github.com/mentiora-ai/loom@v0.13.2#subdirectory=python-sdk"
```
...
loom is **0.13.2** — pre-1.0. The matrix below is the stability

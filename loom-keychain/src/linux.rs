...
//!
//! **W3 stub** — full implementation lands in W3 (Phase 3 Iter 3).
//! Provides the `LinuxKeychain` type so `select_keychain` can construct it
...
impl KeychainAccess for LinuxKeychain {
    fn get_secret(&self, _label: &str) -> Result<Zeroizing<Vec<u8>>, KeychainError> {
        Err(KeychainError::new(
            ...
            "LinuxKeychain not yet implemented (W3 placeholder; v0.9.4 Phase 3 Iter 3)",
        ))
    }
